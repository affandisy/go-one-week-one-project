...
type Attendance struct {
	ID             uuid.UUID  `gorm:"type:uuid;default:gen_random_uuid();primaryKey" json:"id"`
	UserID         uuid.UUID  `gorm:"not null;index:idx_attendance_user_date" json:"user_id"`
	User           User       `gorm:"foreignKey:UserID" json:"user"`
	AttendanceDate time.Time  `gorm:"type:date;not null;index:idx_attendance_user_date" json:"attendance_date"`
	Status         string     `gorm:"not null" json:"status"` // hadir, izin, sakit, alfa
